...
- ACI Neutron plugin (ML2 mechanism driver or GBP — Group Based Policy)
- OpenStack networks map to EPGs, security groups map to Contracts
- Neutron ports automatically become ACI endpoints
- OpFlex protocol for endpoint registration and policy download
- VMM domain integration: APIC discovers VMs and maps to EPGs automatically
- Floating IP / SNAT handled via L3Out with external EPGs
